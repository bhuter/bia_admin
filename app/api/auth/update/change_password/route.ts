...
export async function POST(req: NextRequest): Promise<NextResponse> {
    try {
        const { prevPassword, password, id } = await req.json();

        // Input validation
  
        if (!id || !password || !prevPassword ) {
        ...
        const userResult = await client.query(checkUserSql, [id]);
        ...
        if(userResult.rows[0].password !== await hashPassword(prevPassword)){
          return NextResponse.json({ message: "Incorrect old password." }, { status: 400 });
        }

        if(userResult.rows[0].password === await hashPassword(password)){
          return NextResponse.json({ message: "New password can't be the same with old password" }, { status: 400 });
        }
        // Update user profile
        const updateUserSql = `
            UPDATE users
        ...
        const message = `Your password has been changed. Done at${(new Date()).toLocaleString()} from ${deviceInfo}. \nIf it was not you please let's know at support@biafricantouch.com`;
        ...
        await client.query(notification, [message, id, "Security and Privacy", "true", "Unread", `/dash/profile`, "Unread"])
